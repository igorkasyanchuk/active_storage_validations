...
RSpec.describe ActiveStorageValidations::ContentTypeValidator do
  ...
  describe "#check_validity!" do
    ...
    describe "content type validity" do
      context "when the passed option is an invalid content type" do
        describe ":with" do
          subject(:model) { validator_test_class::CheckValidityInvalidContentTypeWith.new(params) }

          let(:error_message) do
            <<~ERROR_MESSAGE
              You must pass valid content types to the validator
              '#{invalid_content_type}' is not found in Marcel content types (Marcel::TYPE_EXTS + Marcel::MAGIC)
            ERROR_MESSAGE
          end
        ...
        describe ":in" do
          subject(:model) { validator_test_class::CheckValidityInvalidContentTypeIn.new(params) }
        ...
        describe "Edge cases" do
          context "when the passed option is 'image/jpg'" do
            subject(:model) { validator_test_class::CheckValidityInvalidContentTypeJpg.new(params) }
          ...
          context "when the passed option is in Marcel::MAGIC (e.g. 'application/x-ole-storage')" do
      ...
      context "when the passed option is an invalid extension" do
        subject(:model) { validator_test_class::CheckValidityInvalidExtension.new(params) }
      ...
      context "when the passed option is a Regex" do
  ...
  describe "Validator checks" do
    ...
    #     subject(:record) { model.public_send(attribute).attach(file_with_issue) and model }
    ...
    #       subject(:record) { model.public_send(attribute).attach(docx_file_with_two_extensions) and model }
    ...
    #     subject(:record) { model.public_send(attribute).attach(pdf_file_with_extension_in_uppercase) and model }
    ...
    #     subject(:record) { model.public_send(attribute).attach(pdf_file_without_extension) and model }
        ...
        describe value_type do
          %w[string symbol regex].each do |type|
            describe type do
              ...
              context "when provided with an allowed type file" do
              ...
              context "when provided with a not allowed type file" do
                subject(:record) { model.public_send(attribute).attach(not_allowed_file) and model }
    ...
    describe "Edge cases" do
      context "when using a file that has a content_type with a parameter (e.g. 'application/x-rar-compressed;version=5')" do
        subject(:record) { model.public_send(attribute).attach(file_having_content_type_with_parameter) and model }
    ...
    describe ":spoofing_protection" do
      ...
      context "when the protection is enabled (spoofing_protection: true option)" do
        ...
        context "when the file is empty" do
          subject(:record) { model.public_send(attribute).attach(empty_file) and model }
        ...
        context "when the file mime type is not identifiable" do
          subject(:record) { model.public_send(attribute).attach(not_identifiable_type) and model }
        ...
        context "with has_many_attached relationship" do
          let(:attribute) { :many_spoofing_protection }

          context "when the files are okay" do
            subject(:record) { model.public_send(attribute).attach(okay_files) and model }
          ...
          context "when one of the file is spoofed" do
            subject(:record) { model.public_send(attribute).attach(files) and model }
