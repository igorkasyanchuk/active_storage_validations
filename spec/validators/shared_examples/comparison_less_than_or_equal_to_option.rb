...
RSpec.shared_examples "comparison less_than_or_equal_to option" do
  ...
  %w[value proc].each do |value_type|
    describe "#{value_type} validator" do
      let(:attribute) { :"less_than_or_equal_to#{'_proc' if value_type == 'proc'}" }

      context "when provided with a file with a lower value than the value specified in the model validations" do
        subject(:record) { model.public_send(attribute).attach(file_having_lower_than_less_than_or_equal_to_option) and model }

        it { is_expected_to_be_valid }
      end

      context "when provided with a file with the exact value specified in the model validations" do
        subject(:record) { model.public_send(attribute).attach(file_having_exact_less_than_or_equal_to_option) and model }

        it { is_expected_to_be_valid }
      end

      context "when provided with a file with a higher value than the value specified in the model validations" do
        subject(:record) { model.public_send(attribute).attach(file_having_higher_than_less_than_or_equal_to_option) and model }
