...
RSpec.shared_examples "works with unless option" do
  subject(:model) { validator_test_class::WithUnless.new(params) }
  ...
  context "when the :unless condition is a method" do
    context "and the condition is not met" do
      # Here 0 is important to only trigger the #with_unless attribute
      let(:params) { { rating: 0 } }

      context "and when passed a file matching the requirements" do
        before { subject.with_unless.attach(file_matching_requirements) }

        it { is_expected_to_be_valid }
      end

      context "and when passed a file not matching the requirements" do
  ...
  context "when the :unless condition is a Proc" do
    context "and the condition is not met" do
