...
RSpec.shared_examples "works with except_on option" do
  # Rails :except_on is available since Rails 8.0
  if Rails.gem_version >= Gem::Version.new("8.0.0")
    subject(:model) { validator_test_class::WithExceptOn.new(params) }
    ...
    context "when the validation context is excepted" do
      context "when passed a file matching the requirements" do
        before { subject.with_except_on.attach(file_matching_requirements) }

        it { is_expected_to_be_valid(context: :update) }
      end

      context "when passed a file not matching the requirements" do
        before { subject.with_except_on.attach(file_not_matching_requirements) }

        it { is_expected_to_be_valid(context: :update) }
      end
    end

    context "when the validation context is not excepted" do
      context "when passed a file matching the requirements" do
        before { subject.with_except_on.attach(file_matching_requirements) }

        it { is_expected_to_be_valid(context: :create) }
      end

      context "when passed a file not matching the requirements" do
        let(:error_options) { { except_on: :update } }
